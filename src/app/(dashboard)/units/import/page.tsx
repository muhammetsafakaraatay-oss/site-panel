...
export default function ImportUnitsPage() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setPreview(rows)
      setLoading(false)
      showToast('success', `${rows.length} daire yüklendi`)
    }
      ...
      {preview.length > 0 && (
        <div className="bg-[#13161f] border border-white/5 rounded-2xl p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-white font-semibold">Önizleme ({preview.length} daire)</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-2 text-gray-500">Blok</th>
              ...
              <tbody>
                {preview.slice(0, 10).map((item, idx) => (
                  <tr key={idx}>
                    <td className="py-2 text-white">{item.block_name || item['Blok'] || '-'}</td>
